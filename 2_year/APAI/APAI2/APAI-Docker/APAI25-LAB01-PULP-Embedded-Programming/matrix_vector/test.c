...
void stop_perf_counter()
{
	...
	*/
	...
	float cpi = cycles_counter / (float) instr_counter;
	// MAC/Cycles
	float mac_on_cycles = mac_counter / (float) cycles_counter;
	// Instructions/Cycles
	float instructions_on_cycles = instr_counter / (float) cycles_counter;
	// Instructions/MAC
	float instructions_on_mac = instr_counter / (float) mac_counter;
	...
	*/
	...
	for (int i = 0; i < size_N; i++) // outer looop of the gemv
	{
		for (int j = 0; j < size_M; j++) // inner loop of the gemv
		{
			...
			// NOTE: FLOP ≠ MAC. A FLOP is a single floating point operation 
			// (e.g., an addition or a multiplication), while a MAC combines 
			// both operations into one instruction.
			mac_counter += 1; 
...
int main()
{
	...
	for (int i = 0; i < N; i++)					// NOTE: it is Nx1 because you are doing (NxM)@(Mx1) -> (Nx1)
	{
		output_vec[i] = 0;
	}
	printf("\n");
