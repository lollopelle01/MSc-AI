"""
notebooks/anomaly_detection/pipeline_1_2_3/wire_full_pipeline_gmm.py -- identical to wire_full_pipeline.py
but sourcing ANOMALY_SCORE from GMM's keyed HI instead of PCA's, to test
whether GMM performs as well as PCA once wired through the actual pipeline
(not just as a standalone detector).
...
def main():
    ...
    report = "\n".join(lines)
    print(report)
    with open(os.path.join(HERE, "full_pipeline_report_gmm.txt"), "w") as f:
        f.write(report + "\n")
