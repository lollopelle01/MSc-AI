"""
notebooks/anomaly_detection/pipeline_1_2_3/fairness_expgrad_pipeline.py
...
Outputs (written next to this script):
    - fairness_expgrad_report.txt
    - shap_summary_extended_anomaly_expgrad.png
    - shap_summary_core_expgrad.png
    - shap_dependence_anomaly_score_expgrad.png
"""
...
def main():
    ...
    for tier_name, cols in feature_sets.items():
        ...
        print(f"  tier={tier_name} done.", flush=True)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(HERE, "shap_summary_extended_anomaly_expgrad.png"), dpi=150)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(HERE, "shap_dependence_anomaly_score_expgrad.png"), dpi=150)
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(HERE, "shap_summary_core_expgrad.png"), dpi=150)
    plt.close()
    ...
    report = "\n".join(lines)
    print("\n" + report)
    with open(os.path.join(HERE, "fairness_expgrad_report.txt"), "w") as f:
        f.write(report + "\n")
