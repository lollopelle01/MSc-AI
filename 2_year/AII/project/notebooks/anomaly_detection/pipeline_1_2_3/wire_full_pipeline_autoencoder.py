"""
notebooks/anomaly_detection/pipeline_1_2_3/wire_full_pipeline_autoencoder.py -- identical to wire_full_pipeline.py
but sourcing ANOMALY_SCORE from the genuine Keras/TF autoencoder's keyed HI
instead of PCA's, to test whether the autoencoder performs as well as PCA
once wired through the actual pipeline (not just as a standalone detector).
...
Outputs (written next to this script):
    - full_pipeline_report_autoencoder.txt
...
def main():
    ...
    report = "\n".join(lines)
    print(report)
    with open(os.path.join(HERE, "full_pipeline_report_autoencoder.txt"), "w") as f:
        f.write(report + "\n")
